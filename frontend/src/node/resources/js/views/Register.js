...
const Register = () => {
  ...
  const usernameValue = useSelector((state) => state.auth.username);
  const handleUsernameChange = useCallback(
    (event) => dispatch({ type: UPDATE_AUTH_FIELD, key: 'username', value: event.target.value }),
    [dispatch],
  );
  ...
  const submitHandler = (event) => {
    ...
    const payload = agent.Auth.register(usernameValue, emailValue, passwordValue);
    dispatch({ type: REGISTER, payload });
  };

  // 'componentWillUnmount' in react hoooks.
  useEffect(() => () => dispatch({ type: REGISTER_PAGE_UNLOADED }), []);
